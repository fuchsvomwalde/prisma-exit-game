...
export default async function PasswordProtectedLink({
  ...
  params: { gameSlug: string; linkSlug: string; locale: string };
            ...
            {passwordError && (
              <p className="mt-2 text-sm text-red-600 dark:text-red-500 text-shadow-lg shadow-red-800">
                <span className="font-medium">
                  {t("protectedLink.error.title")}
                </span>{" "}
                {t("protectedLink.error.description")}
              </p>
            )}
            <button
              type="submit"
